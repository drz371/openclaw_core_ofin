...
OpenClaw 支持多 Agent 并行运行，默认配置两种 Agent 类型：

| Agent | Agent ID | 端口 | 专长技能 |
|-------|----------|------|----------|
| **OpenClaw** | agent_01 | 50052 | detect_objects, summarize_pdf, process_data |
| **Hermes** | agent_02 | 50053 | analyze_context, generate_response, translate_text |

### 启动命令

```bash
# 启动协调器
clawfed server --addr "0.0.0.0:50051" --agent-id coordinator

# 启动 OpenClaw Agent
clawfed agent --server --addr "0.0.0.0:50052" --agent-id "agent_01"

# 启动 Hermes Agent
clawfed agent --server --addr "0.0.0.0:50053" --agent-id "agent_02"
```
...
clawfed call agent_02 translate_text \
  --addr "http://127.0.0.1:50053" \
  --args '{"text": "Hello", "to": "zh"}'

# Hermes 调用 OpenClaw 的图像识别技能
clawfed call agent_01 detect_objects \
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--agent-id` | agent_01 | Agent 标识符，agent_01=OpenClaw, agent_02=Hermes |
| `--addr` | [::1]:50052 | 服务绑定地址 |
| `--server` | - | 启动为服务器模式 |
...
| **Agent** | `agent/` | Agent 生命周期管理，多 Agent 支持 |
